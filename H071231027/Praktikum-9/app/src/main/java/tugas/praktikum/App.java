...
public class App extends Application {
    ...
    private Scene registerScene(Stage stage) {
        ...
        alamatLabel.getStyleClass().add("input");
        alamatLabel.relocate(90, 300);
        alamatField.relocate(190, 300);
        alamatField.getStyleClass().add("tabelInput");

        Label golongantLabel = new Label("Golongan\t:");
        TextField golonganField = new TextField();
        golongantLabel.getStyleClass().add("input");
        golongantLabel.relocate(90, 340);
        golonganField.relocate(190, 340);
        golonganField.getStyleClass().add("tabelInput");

        Label hobiLabel = new Label("Hobi\t:");
        TextField hobiField = new TextField();
        hobiLabel.getStyleClass().add("input");
        hobiLabel.relocate(90, 400);
        hobiField.relocate(190, 400);
        hobiField.getStyleClass().add("tabelInput");
        ...
        submit.setOnAction(e -> {
            ...
            hobi     = hobiField.getText();
            golongan = golonganField.getText();

            if (name.isEmpty() || date.isEmpty() || username.isEmpty() || password.isEmpty() || alamat.isEmpty() || hobi.isEmpty() || golongan.isEmpty()) {
                ...
                userProfiles.put(username, name + ", Birthday\t: " + date + ", Alamat\t: " + alamat + ", Hobi\t: " + hobi + ", Golongan\t: " + golongan);
        ...
        back.relocate(270, 460);
        back.setOnAction(e -> stage.setScene(mainMenuScene(stage)));

        Pane root = new Pane();
        root.getChildren().addAll(registerLabel, nama1, nama2, dateField, dateLabel, usernama1, usernama2, passwordField, passwordLabel, alamatField, alamatLabel, hobiField, hobiLabel, golonganField, golongantLabel, submit, salah1, back
        );
        root.getStyleClass().add("latar");
    ...
    private Scene profileScene(Stage stage) {
        ...
        Label hasilAlamat = new Label("" + profileDetails[2]);
        hasilAlamat.getStyleClass().add("input");
        hasilAlamat.relocate(90, 270);

        Label hasilGolongan = new Label("" + profileDetails[3]);
        hasilGolongan.getStyleClass().add("input");
        hasilGolongan.relocate(90, 320);

        Label hasilHobi = new Label("" + profileDetails[4]);
        hasilHobi.getStyleClass().add("input");
        hasilHobi.relocate(90, 390);
        ...
        Pane root = new Pane();
        root.getChildren().addAll(profil, back, hasilNama, hasilDate, hasilAlamat, hasilGolongan, hasilHobi);
        root.getStyleClass().add("latar");
        Scene scene = new Scene(root, 640, 480);
        scene.getStylesheets().add(getClass().getResource("/Style/Style.css").toExternalForm());
        return scene;
